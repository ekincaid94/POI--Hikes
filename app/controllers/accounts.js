...
const Accounts = {
  index: {
    ...
    handler: function (request, h) {
      return h.view("main", { title: "Welcome to The Hike Life" });
    },
  },
  showSignup: {
    auth: false,
  ...
  signup: {
    ...
    validate: {
      payload: {
        firstName: Joi.string()
          .required()
          .regex(/^[A-Z][a-z]{2,30}$/), // begin with upper case letter and then 2 -30 lower case
        lastName: Joi.string()
          .required()
          .regex(/^[A-Z][a-z]{2,30}$/), // begin with upper case letter and then 2 -30 lower case
    ...
    handler: async function (request, h) {
      try {
        ...
        if (user) {
          const message = "This Email address is already registered in the system";
  ...
  showSettings: {
    handler: async function (request, h) {
      try {
        ...
        return h.view("settings", { title: "POI Account Settings", user: user });
      } catch (err) {
